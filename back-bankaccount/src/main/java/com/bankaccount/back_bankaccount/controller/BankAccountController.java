...
public class BankAccountController {
    ...
    public ResponseEntity<BankAccountDto> setOverdraftLimit(@Valid @RequestBody OverdraftRequestDto request) {
        log.info("Overdraft limit request: {} for account {}", 
            request.getOverdraftLimit(), request.getAccountNumber());
        BankAccountDto result = bankAccountService.setOverdraftLimit(request.getAccountNumber(), request.getOverdraftLimit());
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<BankAccountDto> depositToSavings(@Valid @RequestBody DepositRequestDto request) {
        log.info("Savings deposit request: {} amount {} on account {}", 
            request.getAmount(), request.getAccountNumber());
        BankAccountDto result = bankAccountService.depositToSavings(request.getAccountNumber(), request.getAmount());
        return ResponseEntity.ok(result);
    }

    @GetMapping(value = ResourcePath.PATH_STATEMENT + "/{accountNumber}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<StatementDto> getStatement(@PathVariable String accountNumber) {
        log.info("Statement request for account {}", accountNumber);
        StatementDto result = bankAccountService.getStatement(accountNumber);
        return ResponseEntity.ok(result);
    }
}