...
describe('AccountDashboard', () => {
  ...
  beforeEach(async () => {
    ...
    // Create a manual mock for AccountContextService
    const mockContextService: Partial<AccountContextService> = {
    ...
    mockAccountContextService = mockContextService as jasmine.SpyObj<AccountContextService>;
  ...
  describe('Navigation et boutons', () => {
    ...
    it('should display "Dépôt d\'argent" operation item', () => {
      ...
      const depositElement = Array.from(depositElements).find(
        (el) => el.textContent?.trim() === "Dépôt d'argent"
      );
    ...
    it('should display "Retrait d\'argent" operation item', () => {
      ...
      const withdrawalElement = Array.from(withdrawalElements).find(
        (el) => el.textContent?.trim() === "Retrait d'argent"
      );
  ...
  describe('UX et accessibilité', () => {
    ...
    it('should have proper CSS classes for operation items', () => {
      ...
      expect(operationItems.length).toBe(3); // Dépôt, Retrait, Relevé de Compte
